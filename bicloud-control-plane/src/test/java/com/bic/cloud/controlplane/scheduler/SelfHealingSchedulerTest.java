...
class SelfHealingSchedulerTest {
    ...
    @Test
    @DisplayName("reconcile -> calls deploy when running count is below desired")
    void reconcile_deploysWhenRunningCountBelowDesired() {
        ProjectImage image = buildImage("web", 3);
    ...
    void reconcile_continuesAfterDeploymentFailure() {
        ProjectImage failingImage = buildImage("broken-svc", 2);
        ProjectImage healthyImage = buildImage("healthy-svc", 1);
        ...
        // the first deploy throws
        doThrow(new RuntimeException("Worker unavailable"))
                .when(deploymentService).deploy(failingImage);

        scheduler.reconcile();

        // deploy must have been called for both images
        verify(deploymentService).deploy(failingImage);
        verify(deploymentService).deploy(healthyImage);
    }
    ...
    @Test
    @DisplayName("reconcile -> does not deploy when running count exceeds desired")
    void reconcile_doesNotDeployWhenRunningExceedsDesired() {
        ProjectImage image = buildImage("api", 1);
    ...
    void reconcile_entersCooldownOnCrashLoop() {
        ProjectImage image = buildImage("crashing-svc", 1);
    ...
    void reconcile_deploysWhenFailureCountBelowCrashLoopThreshold() {
        ProjectImage image = buildImage("flaky-svc", 1);
    ...
    private ProjectImage buildImage(String serviceName, int desiredReplicas) {
